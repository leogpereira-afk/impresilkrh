  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-4 xl:grid-cols-2">
      <Card><CardHeader title="Evolução do pago à equipe" subtitle="Meses sem registros ficam identificados na tabela"/><CardBody>
        {r.meses.some(m=>m.pago!=null)?<BarrasVerticais moeda vazio="R$ 0,00 pago à equipe nos meses com registro." data={r.meses.filter(m=>m.pago!=null).map(m=>({nome:compLabel(m.competencia),valor:m.pago!}))}/>:<p className="text-sm text-slate-500">Nenhum registro no período.</p>}
    ...
    <Card><CardHeader title="Distribuição por área" subtitle="Cadastro atual · valores pagos à equipe"/><CardBody><div className="rh-stats">{r.porArea.map(a=><div key={a.nome} className="rounded-xl border border-slate-200 p-3"><p className="text-sm text-slate-500">{areas.find(x=>x.id===a.nome)?.nome??'Sem área / sem cadastro'}</p><strong>{formatBRL(a.valor)}</strong></div>)}</div></CardBody></Card>
